...
`ParamCartPoleEnv` in tests/test_grader.py. The policy is deterministic
...
from acl_bench.cartpole import MAX_EPISODE_STEPS, PARAM_ORDER
...
_LENGTH, _MASSPOLE, _MASSCART, _FORCE = (PARAM_ORDER.index(k) for k in ("length", "masspole", "masscart", "force_mag"))
...
@torch.no_grad()
def rollout_steps(agent, params: np.ndarray, s0: np.ndarray, max_steps: int = MAX_EPISODE_STEPS) -> np.ndarray:
