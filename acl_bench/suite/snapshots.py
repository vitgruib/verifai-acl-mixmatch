...
`run_arms --snapshots DIR` writes one file per run, `DIR/<arm>/<replicate>.npz`,
holding the agent's weights at every check-in (about 37 KB each). Any exam, including
sections built after training (E6/E7), can then be graded against the saved agents with
`python -m acl_bench.suite.regrade`, without retraining.
"""
...
OBS_DIM, ACTION_DIM = 4, 2          # CartPole
...
def load_run(path: str) -> dict[int, Agent]:
    ...
    for step in sorted(states):
        agent = Agent(OBS_DIM, ACTION_DIM, "discrete")
