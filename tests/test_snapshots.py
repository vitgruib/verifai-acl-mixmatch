...
def test_saved_snapshots_regrade_to_exactly_the_live_grades(tmp_path):
    live = pd.DataFrame(run_job(("cartpole", "A", 3, 4096, 2048, EXAM, str(tmp_path))))
    ...
    again = pd.DataFrame(grade_run(CARTPOLE, "A", 3, path, load_sets(EXAM)))
...
def test_regrade_on_a_checkpoint_grid_keeps_the_final_model(tmp_path):
    run_job(("cartpole", "N", 1, 6144, 1024, EXAM, str(tmp_path)))       # check-ins at 0, 1024, ..., 6144
    rows = grade_run(CARTPOLE, "N", 1, str(tmp_path / "N" / "1.npz"), load_sets(EXAM), n_checkpoints=3)
    assert [r["step"] for r in rows] == [2048, 4096, 6144]


def test_snapshots_can_be_graded_on_a_different_exam_later(tmp_path):
    run_job(("cartpole", "N", 1, 2048, 2048, EXAM, str(tmp_path / "snaps")))
    small = tmp_path / "exam"
    os.makedirs(small)
    ...
    only = grade_run(CARTPOLE, "N", 1, str(tmp_path / "snaps" / "N" / "1.npz"), load_sets(str(small)))
    assert all("random/success" in r and "verifai/success" not in r for r in only)


def test_snapshots_are_small(tmp_path):
    run_job(("cartpole", "N", 1, 4096, 2048, EXAM, str(tmp_path)))
